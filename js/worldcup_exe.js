...
function ready(type) {
	...
	//8. 출전 인원 표시
		...
		for (var i=0;i<arr_go.length;i++) {
			...
			//b-2. 이미지 사이즈 조절
			img.style.width = ((img.naturalWidth/img.naturalHeight) * 120).toString() + "px";
	...
	$("#ready_start").onclick = function() {
		...
		$("#battle_img_left").style.left = "-285px";
		$("#battle_img_right").style.right = "-285px";
		//7-2. 프레임 전환
...
function battle_ready(counter){
	if (counter == 0) {
		...
		//3. 이미지 표시
			//3-1. 좌측 이미지
				...
				//c. 이미지 사이즈 조절
				img.style.width = ((img.naturalWidth/img.naturalHeight) * 300).toString() + "px";
	...
	if (counter < 260) {
		//중간 : 캐릭터 이동
		//1. 이미지 이동
		$("#battle_img_left").style.left = ($("#battle_img_left").offsetLeft + 40).toString() + "px";
		$("#battle_img_right").style.right = (-($("#battle_img_right").offsetWidth + $("#battle_img_right").offsetLeft - 600) + 40).toString() + "px";
		
		//2. 다음 이동 대기
		setTimeout(function() {
			battle_ready(counter + 40);
		},1000/60);
		
	} else {
		//나머지 : 전투 실시 (클릭 대기)
		//1. 이미지 정지
		$("#battle_img_left").style.left = "10px";
		$("#battle_img_right").style.right = "10px";
		//2. 이미지선택 효과 활성화
...
window.onload = function() {
	//이미지 선로딩 - 준비
	imageList.push(images + "frame/mark_vs.png");
	imageList.push(images + "frame/logo.png");
	...
	$("#loading_start").onclick = function() {
		...
		loadImages(imageList,function() {
			...
			$("#intro_start_2").onclick = function() {
				ready("남자 직업");
			}
			$("#intro_start_3").onclick = function() {
				ready("남자");
			}
			
			$("#intro_start_4").onclick = function() {
				ready("여자 NPC");
			}
			$("#intro_start_5").onclick = function() {
				ready("여자 직업");
			}
			$("#intro_start_6").onclick = function() {
				ready("여자");
			}
			
			$("#intro_start_7").onclick = function() {
				ready("all");
			}
