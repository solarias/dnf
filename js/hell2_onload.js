	...
	loadImages(imageList,function(){
		...
		//final. 조건부 실행, 필터, 체크박스, 버튼 세팅
				...
				for (i=0;i<$objective_list.options.length;i++) {
					if($objective_list.options[i].value != "") {//구분선 제외
						if ($objective_list.options[i].value == $objective_list.value) {
							document.getElementById("objective_" + $objective_list.options[i].value).style.display = "block";
						} else {
							document.getElementById("objective_" + $objective_list.options[i].value).style.display = "none";
						}
					}
				}
				//리스트 색상 변경
			...
			$objective_set_name.onchange = function() {
				...
				for (i=0;i<temp2;i++) {
					if (document.getElementById("set_row_" + i.toString()).getElementsByTagName("td")[0].innerHTML.indexOf($objective_set_name.value) != -1) {
		...
		//final-1-1-1. record 필터링
			$record_filter_first.onchange = function() {
				var sheet = document.getElementById("style_record_filter_first");
				var value = $record_filter_first.value;
				try {
					//1. 모든 필터링 해제 후 두번째, 세번째 '무조건' 초기화
						...
						//나머지 필터링 선택지 초기화
						$record_filter_second.selectedIndex = 0;
						$record_filter_third.selectedIndex = 0;
						//나머지 필더링 비활성화
						$record_filter_second.disabled = "disabled";
						$record_filter_third.disabled = "disabled";
						//select 배경색 변경
						$record_filter_first.style.backgroundColor = "white";
						$record_filter_second.style.backgroundColor = "white";
						$record_filter_third.style.backgroundColor = "white";
						//두번째 필터링 비우고 새로 입력
						var dropdown = $record_filter_second;
						clearSelect(dropdown);
						...
						dropdown.add(option);
						//세번째 필터링 비우고 새로 입력
						var dropdown = $record_filter_third;
						clearSelect(dropdown);
					...
					if ($record_filter_first.value != "") {
						//필터링 적용
						sheet.innerHTML = "\
						#record span.equip:not(." + value + ") {\
							display:none;\
						}\
						#record span.run:not(." + value + ") {\
							display:none;\
						}\
						#record span.get:not(." + value + ") {\
							display:none;\
						}";
						//select 배경색 변경
						$record_filter_first.style.backgroundColor = "#F4FA58";
						//두번째 필터링 활성화
						$record_filter_second.disabled = "";
					...
					$record_filter_first.selectedIndex = 0;
				}
			}
			
			$record_filter_second.onchange = function() {
				var sheet = document.getElementById("style_record_filter_second");
				var value = $record_filter_second.value;
				try {
					...
					if ($record_filter_second.value != "") {
						...
						//select 배경색 변경
						$record_filter_second.style.backgroundColor = "#F4FA58";
						//두번째 필터링 활성화
						$record_filter_third.disabled = "";
			...
			$record_filter_third.onchange = function() {
				var sheet = document.getElementById("style_record_filter_second");
				var value = $record_filter_third.value;
				try {
					if ($record_filter_third.value != "") {
						//필터링 적용
						sheet.innerHTML = "\
						#record span.equip:not(." + value + ") {\
							display:none;\
						...
						//select 배경색 변경
						$record_filter_third.style.backgroundColor = "#F4FA58";
					} else {
						//필터링 해제
						sheet.innerHTML = "";
						//select 배경색 복구
			...
			$record_filter_level.onchange = function() {
				var sheet = document.getElementById("style_record_filter_level");
				var value = "lv" + $record_filter_level.value;
				try {
					//필터링 적용
					if ($record_filter_level.value != "") {
						sheet.innerHTML = "\
						#record span.equip:not(." + value + ") {\
							display:none;\
						}";
						//select 배경색 변경
						$record_filter_level.style.backgroundColor = "#F4FA58";
					} else {
						sheet.innerHTML = "";
						//select 배경색 복구
						$record_filter_level.style.backgroundColor = "white";
					}
				} catch(e) {
					alert("＊경고 : \"레벨 분류\" 필터링을 사용할 수 없습니다.\n(브라우저가 특정 기능을 지원하지 않습니다.)");
					$record_filter_level.selectedIndex = 0;
				}
			}
			
			$record_filter_clear.onclick = function() {
				//필터링 해제
				document.getElementById("style_record_filter_first").innerHTML = "";
				document.getElementById("style_record_filter_second").innerHTML = "";
				document.getElementById("style_record_filter_third").innerHTML = "";
			...
			$inventory_filter_first.onchange = function() {
				var sheet = document.getElementById("style_inventory_filter_first");
				var value = $inventory_filter_first.value;
				try {
					//1. 모든 필터링 해제 후 두번째, 세번째 '무조건' 초기화
						//필터링 해체
						...
						//나머지 필터링 선택지 초기화
						$inventory_filter_second.selectedIndex = 0;
						$inventory_filter_third.selectedIndex = 0;
						//나머지 필더링 비활성화
						$inventory_filter_second.disabled = "disabled";
						$inventory_filter_third.disabled = "disabled";
						//select 배경색 변경
						$inventory_filter_first.style.backgroundColor = "white";
						$inventory_filter_second.style.backgroundColor = "white";
						$inventory_filter_third.style.backgroundColor = "white";
						//두번째 필터링 비우고 새로 입력
						var dropdown = $inventory_filter_second;
						clearSelect(dropdown);
						...
						dropdown.add(option);
						//세번째 필터링 비우고 새로 입력
						var dropdown = $inventory_filter_third;
						clearSelect(dropdown);
						...
						dropdown.add(option);
					//2. 첫번째 값이 있으면 (필터링 후) 두번째 생성
					if ($inventory_filter_first.value != "") {
						//필터링 적용
						sheet.innerHTML = "\
						...
						//select 배경색 변경
						$inventory_filter_first.style.backgroundColor = "#F4FA58";
						//두번째 필터링 활성화
						$inventory_filter_second.disabled = "";
						//첫번째 필터를 기준으로 두번째 필터링 구성
						var tempArr = [];
						for (i=0;i<itemList.length;i++) {
							//2차 기준 (1차 선택지가 있어야, 텅빈 건 제외)
					...
					alert("＊경고 : \"1차 분류\" 필터링을 사용할 수 없습니다.\n(브라우저가 특정 기능을 지원하지 않습니다.)");
					$inventory_filter_first.selectedIndex = 0;
				}
			}
			
			$inventory_filter_second.onchange = function() {
				var sheet = document.getElementById("style_inventory_filter_second");
				var value = $inventory_filter_second.value;
				try {
					//1. 모든 필터링 해제 후 세번째 '무조건' 초기화
						//필터링 해체
						document.getElementById("style_inventory_filter_second").innerHTML = "";
					...
					//2. 두번째 값이 있으면 (필터링 후) 세번째 생성
					if ($inventory_filter_second.value != "") {
						//필터링 적용
						sheet.innerHTML = "\
						#inventory_display table tr:not(." + value + ") {\
							display:none;\
						...
						//select 배경색 변경
						$inventory_filter_second.style.backgroundColor = "#F4FA58";
						//두번째 필터링 활성화
						$inventory_filter_third.disabled = "";
						//두번째 필터를 기준으로 세번째 필터링 구성
						var tempArr = [];
						for (i=0;i<itemList.length;i++) {
							//3차 기준 (2차 선택지가 있어야, 텅빈 건 제외)
			...
			$inventory_filter_third.onchange = function() {
				var sheet = document.getElementById("style_inventory_filter_second");
				var value = $inventory_filter_third.value;
				try {
					//필터링 적용
					if ($inventory_filter_third.value != "") {
						sheet.innerHTML = "\
						#inventory_display table tr:not(." + value + ") {\
							display:none;\
						}";
						//select 배경색 변경
						$inventory_filter_third.style.backgroundColor = "#F4FA58";
					} else {
						//필터링 해제
						sheet.innerHTML = "";
						//select 배경색 복구
			...
			$inventory_filter_level.onchange = function() {
				var sheet = document.getElementById("style_inventory_filter_level");
				var value = "lv" + $inventory_filter_level.value;
				try {
					//필터링 적용
					if ($inventory_filter_level.value != "") {
						sheet.innerHTML = "\
						#inventory_display table tr:not(." + value + ") {\
							display:none;\
						}";
						//select 배경색 변경
						$inventory_filter_level.style.backgroundColor = "#F4FA58";
					} else {
						sheet.innerHTML = "";
						//select 배경색 복구
						$inventory_filter_level.style.backgroundColor = "white";
					}
				} catch(e) {
					alert("＊경고 : \"레벨 분류\" 필터링을 사용할 수 없습니다.\n(브라우저가 특정 기능을 지원하지 않습니다.)");
					$inventory_filter_level.selectedIndex = 0;
				}
			}
			
			$inventory_filter_clear.onclick = function() {
				//필터링 해제
				document.getElementById("style_inventory_filter_first").innerHTML = "";
				document.getElementById("style_inventory_filter_second").innerHTML = "";
				document.getElementById("style_inventory_filter_third").innerHTML = "";
			...
			//final-1-1-4. craft 필터링
			$craft_filter_first.onchange = function() {
				var sheet = document.getElementById("style_craft_filter_first");
				var value = $craft_filter_first.value;
				try {
					//1. 모든 필터링 해제 후 두번째, 세번째 '무조건' 초기화
						//필터링 해체
						document.getElementById("style_craft_filter_first").innerHTML = "";
						document.getElementById("style_craft_filter_second").innerHTML = "";
						document.getElementById("style_craft_filter_third").innerHTML = "";
						//나머지 필터링 선택지 초기화
						$craft_filter_second.selectedIndex = 0;
						$craft_filter_third.selectedIndex = 0;
						//나머지 필더링 비활성화
						$craft_filter_second.disabled = "disabled";
						$craft_filter_third.disabled = "disabled";
						//select 배경색 변경
						$craft_filter_first.style.backgroundColor = "white";
						$craft_filter_second.style.backgroundColor = "white";
						$craft_filter_third.style.backgroundColor = "white";
						//두번째 필터링 비우고 새로 입력
						var dropdown = $craft_filter_second;
						clearSelect(dropdown);
						...
						dropdown.add(option);
						//세번째 필터링 비우고 새로 입력
						var dropdown = $craft_filter_third;
						clearSelect(dropdown);
						...
						dropdown.add(option);
					//2. 첫번째 값이 있으면 (필터링 후) 두번째 생성
					if ($craft_filter_first.value != "") {
						//필터링 적용
						sheet.innerHTML = "\
						#craft_display table tr:not(." + value + ") {\
							display:none;\
						}";
						//select 배경색 변경
						$craft_filter_first.style.backgroundColor = "#F4FA58";
						//두번째 필터링 활성화
						$craft_filter_second.disabled = "";
						//첫번째 필터를 기준으로 두번째 필터링 구성
						var tempArr = [];
						for (i=0;i<itemList.length;i++) {
							//2차 기준 (1차 선택지가 있어야, 텅빈 건 제외)
					...
					alert("＊경고 : \"1차 분류\" 필터링을 사용할 수 없습니다.\n(브라우저가 특정 기능을 지원하지 않습니다.)");
					$craft_filter_first.selectedIndex = 0;
				}
			}
			
			$craft_filter_second.onchange = function() {
				var sheet = document.getElementById("style_craft_filter_second");
				var value = $craft_filter_second.value;
				try {
					//1. 모든 필터링 해제 후 세번째 '무조건' 초기화
						//필터링 해체
						document.getElementById("style_craft_filter_second").innerHTML = "";
					...
					if ($craft_filter_second.value != "") {
						...
						//select 배경색 변경
						$craft_filter_second.style.backgroundColor = "#F4FA58";
						//두번째 필터링 활성화
						$craft_filter_third.disabled = "";
						//두번째 필터를 기준으로 세번째 필터링 구성
						var tempArr = [];
						for (i=0;i<itemList.length;i++) {
							//3차 기준 (2차 선택지가 있어야, 텅빈 건 제외)
			...
			$craft_filter_third.onchange = function() {
				var sheet = document.getElementById("style_craft_filter_second");
				var value = $craft_filter_third.value;
				try {
					//필터링 적용
					if ($craft_filter_third.value != "") {
						sheet.innerHTML = "\
						#craft_display table tr:not(." + value + ") {\
							display:none;\
						}";
						//select 배경색 변경
						$craft_filter_third.style.backgroundColor = "#F4FA58";
					} else {
						//필터링 해제
						sheet.innerHTML = "";
						//select 배경색 복구
			...
			$craft_filter_level.onchange = function() {
				var sheet = document.getElementById("style_craft_filter_level");
				var value = "lv" + $craft_filter_level.value;
				try {
					//필터링 적용
					if ($craft_filter_level.value != "") {
						sheet.innerHTML = "\
						#craft_display table tr:not(." + value + ") {\
							display:none;\
						}";
						//select 배경색 변경
						$craft_filter_level.style.backgroundColor = "#F4FA58";
					} else {
						sheet.innerHTML = "";
						//select 배경색 복구
						$craft_filter_level.style.backgroundColor = "white";
					}
				} catch(e) {
					alert("＊경고 : \"레벨 분류\" 필터링을 사용할 수 없습니다.\n(브라우저가 특정 기능을 지원하지 않습니다.)");
					$craft_filter_level.selectedIndex = 0;
				}
			}
			
			$craft_filter_clear.onclick = function() {
				//필터링 해제
				document.getElementById("style_craft_filter_first").innerHTML = "";
				document.getElementById("style_craft_filter_second").innerHTML = "";
				document.getElementById("style_craft_filter_third").innerHTML = "";
				document.getElementById("style_craft_filter_level").innerHTML = "";
				//필터링 선택지 복구
				$craft_filter_first.selectedIndex = 0;
				$craft_filter_second.selectedIndex = 0;
				$craft_filter_third.selectedIndex = 0;
			...
			$craft_check_available.onclick = function() {
				var sheet = document.getElementById("style_craft_check_available");
				try {
					if ($craft_check_available.checked) {
						...
						if ($craft_check_all.checked) {
							sheet = document.getElementById("style_craft_check_all");
							sheet.innerHTML = "";
			...
			$craft_check_all.onclick = function() {
				var sheet = document.getElementById("style_craft_check_all");
				try {
					if ($craft_check_all.checked) {
						...
						if ($craft_check_available.checked) {
							sheet = document.getElementById("style_craft_check_available");
							sheet.innerHTML = "";
							$craft_check_available.checked = false;
						}
					} else {
...
\n(코스모소울 보유량 증가 : " + thousand(output) + "개)";
		...
		$filter_name_normal.onclick = function() {
			...
			try {
				...
				if ($filter_name_normal.checked) {
					...
					//필드 아이템 이름 재배치
					for (var i=0;i<maxQuantity;i++) {
						document.getElementById("description" + i.toString()).style.left = (-document.getElementById("item_name" + i.toString()).offsetWidth/2).toString() + "px";
					}
				} else {
					//필터링 해제
					sheet.innerHTML = "";
		...
		//3. 날짜 환산
			...
			$date_config.onclick = function() {
				switch ($date_config.className) {
					...
					case "cancel":
						//a. 설정값 복구
						for (var i = 1;i <= 7;i++) {
							window["$date_setting_" + i.toString()].value = dateSettingDefault[i-1].toString();
						}
			...
			for (var i = 1;i <= 7;i++) {
				window["$date_setting_" + i.toString()].onfocus = function() {
					this.select();
				}
			}
			//날짜환산 - 설정 적용
			$date_apply.onclick = function() {
				//a. 설정값 적용
					...
					for (var i = 1;i <= 7;i++) {
						...
						if (! isNumber(window["$date_setting_" + i.toString()].value)) {
					...
					//a-2. 제대로 입력됨 -> 적용
					for (var i = 1;i <= 7;i++) {
						dateSettingList[i-1] = window["$date_setting_" + i.toString()].value;
					}
					//a-3. 월/년 환산 적용
			...
			$date_cancel.onclick = function() {
				//a. 설정값 복구
				for (var i = 1;i <= 7;i++) {
					window["$date_setting_" + i.toString()].value = dateSettingDefault[i-1];
				}
				switch($date_summary.style.display) {
					case "inline":
						$date_setting_summary.selectedIndex = 1;
